...
namespace HUtil.UI
{
    ...
    public class PropertyBindingPort<T> : IBindingPort
    {
        ...
        private List<string> GetPossiblePaths(MonoBinder binder) => binder.GetAllBindablePropertyNamesEditor(ReceivingType, _direction).ToList();
        ...
        public IViewModelProperty<T> Bind(Dictionary<string, IViewModelProperty> bindMap, CompositeDisposable disposable, Action<T> setter, UnityEvent<T> onChange)
        {
            ...
            Debug.Log($"Try Bind Port {Path} -> {property.ToStringChain()}");
